...
function findWordFromStartingLetter(board, word, y, x, seen) {
  ...
  if (y > 0 && findWordFromStartingLetter(board, word.slice(1), y - 1, x, seen)) return true;
  if (y < 4 && findWordFromStartingLetter(board, word.slice(1), y + 1, x, seen)) return true;
  if (x > 0 && findWordFromStartingLetter(board, word.slice(1), y, x - 1, seen)) return true;
  if (x < 4 && findWordFromStartingLetter(board, word.slice(1), y, x + 1, seen)) return true;

  return false;
}


function checkForWord(board, word) {

  for (let y = 0; y < 5; y++)
    for (let x = 0; x < 5; x++)
      if (findWordFromStartingLetter(board, word, y, x, new Set())) return true;
...
console.log(checkForWord(board2, "NOOOOS"), true);
